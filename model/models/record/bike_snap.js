...
const connection = require('../../connections').record;
...
const schema = new Schema({
  ...
  location: {
    ...
    // 是否异常速度
    isIllegalSpeed: Boolean,
  },
  // 位置业务数据
  gis: {
    // 运营范围外最小不可用距离：缓冲区
    minInvalidDistanceOutServiceArea: Number,
    // 运营范围内最小安全距离
    minValidDistanceInServiceArea: Number,
    // 计算的位置信息
    // 是否在服务区缓冲区外
    isOutServiceBufferArea: Boolean,
    // 是否在服务区外
    isOutServiceArea: Boolean,
    // 是否在服务区内
    isInServiceArea: Boolean,
    // 是否在不合法区域内
    isInInvalidArea: Boolean,
    // 是否不在合法区域内
    isOutValidArea: Boolean,
  ...
  battery: {
    ...
    // 最近未断电时的电压
    latestLinkVoltage: Number,
    // 最近为断电时的电量
    latestLinkPower: Number,
  ...
  biz: {
    ...
    locate: { type: Number, enums: constants.CORE_BIKE_LOCATE_ENUMS, default: constants.CORE_BIKE_LOCATE.在库 },
    // 丢失时间
    lostAt: Date,
    // 丢失后是否扫码
...
schema.index({ no: 1, project: 1, createdAt: -1 });

module.exports = connection.model('bike_snap', schema);